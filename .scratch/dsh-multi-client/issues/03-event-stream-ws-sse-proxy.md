...
- [x] 连接空闲挂起 10 分钟以上不被中间件掐断（实测 4 分钟空闲不断连、之后事件仍到达；未做满 10 分钟，见 Comments）
- [x] 断开后客户端指数退避重连可恢复
...
- `gateway/` 新增依赖 `@fastify/websocket` 11 + `ws` 8；实现集中在 `gateway/src/server.js`：
  - 对 `/api/events.mux`、`/api/events.host` 注册 `{ wsHandler }` 路由手动处理 upgrade（先于 http-proxy 通配注册，静态路由优先匹配）；网关作为 ws 客户端连上游同路径，双向逐帧转发，close/error 互传（保留码 1005/1006/1015 映射 1001）。协议行为原样保留：客户端上行帧 → 上游 `close(1008, 'downlink only')` 经透传回客户端（已验证）。
  - 普通 GET 同路径回 `426 upgrade required`，镜像上游行为（上游 web 面无 SSE 回退，01 实测）。
  - 长连接约束：fastify `connectionTimeout: 0` + `requestTimeout: 0`（Node 默认 requestTimeout 300s 会杀慢请求）；`perMessageDeflate: false` 双向关闭压缩，帧到达即转发无 buffering；`maxPayload = 100 * 1024 * 1024`（100 MiB）同时设在 @fastify/websocket 服务端与上行 ws 客户端（`WS_MAX_PAYLOAD` 常量）。
  - 栅栏头改写抽出 `rewriteHeaders()` 与 02 HTTP 反代共用（Host 钉上游回环、剥 Origin/Sec-Fetch-*/Expect）；WS 方向另剥 `connection`/`upgrade`/`sec-websocket-*` 握手头由 ws 客户端自行生成，子协议透传。
...
- **空闲挂起**：`probe-ws.mjs idle 240` → **空闲 240s（4 分钟）后连接仍 OPEN，之后触发事件收到 9 帧**，exit=0。**未做满 issue 所写的 10 分钟**；4 分钟内无任何中间件掐断（网关侧已关 connectionTimeout/requestTimeout，无代理层 buffering）。
- **指数退避重连**：`probe-ws.mjs reconnect`（1s→2s→4s…退避循环）挂起期间杀网关、占死 3000 端口约 8s 后重启（模拟重启窗口）→ 日志：`连接断开 code=1006，1000ms 后重连 → 连接被拒 2000ms → 连接被拒 4000ms → 第 4 次尝试连接建立 → 本代连接收到 12 帧（下行可用）`，共验证 2 代连接，exit=0。
...
- 空闲挂起实测 4 分钟（建议值 3–5 分钟区间），未满 issue 原文 10 分钟；如需更长时间证据可跑 `node scripts/probe-ws.mjs idle 600`。
