...
- **环境探测结论**：
  - 无 Rust → 已用 rustup 装到用户目录（stable 1.97.1，`~/.cargo`）。
  - 无 sudo（`sudo -n` 需密码）；系统无 pkg-config、无 webkit2gtk-4.1/gtk-3/glib 开发包（`dpkg -l` 仅 build-essential；运行时库 `libgtk-3-0`/`libglib-2.0-0` 在，但无 `-dev`）。
  - **绕行**：`apt-get download` + `dpkg-deb -x` 把 pkg-config/pkgconf 与完整构建依赖树（494 包，含 `libwebkit2gtk-4.1-dev` 2.52.3）解到用户级 sysroot（`/tmp/sysroot`，仓库外、不入 git），`PKG_CONFIG_PATH` + `PKG_CONFIG_SYSROOT_DIR` 指过去后构建全绿。正常机器上一行 `sudo apt install pkg-config libwebkit2gtk-4.1-dev ...` 即可（README 已写）。
- **构建结果**：
  - `cargo check` 通过（tauri 2.11.5 / tauri-plugin-store 2.4.4 / webkit2gtk 2.0.2 全树编译）。
  - `cargo test` 通过：3 个 URL 校验单元测试全绿。
...
1. **首次启动配置页 / 保存后加载网关** — 未验证（headless 无显示器，二进制无法起 GUI）。替代：`set_server_url`/`get_server_url` 逻辑代码审查 + URL 校验单测 + 完整构建通过；窗口 URL 决策（External vs App）为标准 Tauri API 直译。
2. **webview 种 cookie / 重启保持登录** — 未验证（同上，需起 webview 登录）。替代：cookie 模型复用 issue 04 已验证的网关行为（`Secure` 在 https 入口下成立），壳侧零额外代码；WebKitGTK 对 `HttpOnly` cookie 的磁盘持久化为 webview 默认行为。运行前置：Caddy 本地 CA 根证书需被系统信任（同 RUNBOOK 浏览器信任一节），否则 webview 报证书错误。
...
- 四项 GUI 验收需在有显示器的机器上补测：装系统依赖 → `pnpm install && pnpm dev`，入口 `https://localhost:8443`（先 `caddy trust`）。
