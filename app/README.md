# dsh-app — Tauri 2 桌面壳（issue 07 MVP + 08 桌面体验项 + 09 出包/更新 + 12 移动端）
...
同一 `app/` 工程开 Android target。桌面专属能力按平台门控（`#[cfg(desktop)]` +
...
- `endpoints` — `https://updates.dsh.example.com/dsh-app/{{target}}/{{arch}}/{{current_version}}`
...
- `pubkey` — `PLACEHOLDER-REPLACE-WITH-REAL-MINISIGN-PUBLIC-KEY` 为占位。生成密钥对：
  `pnpm exec tauri signer generate -w ~/.tauri/dsh-app.key`（私钥入 CI secrets
